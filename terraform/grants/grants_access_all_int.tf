...
resource "snowflake_grant_account_role" "grant_schema_access_to_all_intermediate_access_sr" {
  provider         = snowflake.useradmin
  for_each         = local.all_intermediate_roles_access_config
  role_name        = "${each.value.database_name}_${each.value.schema_name}_SR"
  parent_role_name = "${each.value.database_name}_ALL_SCHEMAS_SR"
}

resource "snowflake_grant_account_role" "grant_schema_access_to_all_intermediate_access_srw" {
  provider         = snowflake.useradmin
  for_each         = local.all_intermediate_roles_access_config
...
resource "snowflake_grant_account_role" "grant_schema_access_to_all_intermediate_access_sfull" {
  provider         = snowflake.useradmin
  for_each         = local.all_intermediate_roles_access_config
