...
#### Compliance check
In order to run compliance checks locally, you first need to login to ECR. The following command will handle that for you. 
````sh
make compliance
````
...
2. You need to setup an OIDC IDP in every AWS account that is a target for deployments ([see](https://cloud.nwse.io/how-to/github-actions-integration.html))
